...
type
  tmainfo = class(tmainform)
    ...
    procedure mainfo_onclose(const sender: TObject);
    procedure mainfo_onterminatequery(var terminate: Boolean);
    procedure mainfo_onterminated(const sender: TObject);
  private
    fsource: TPAOggVorbisDecoderSource;
...
begin
 ...
 {sd_info.value := unicodeformat('Track %d / %d', [ffileindex + 1, length(ffilelist)]);}
  ...
  fsource := TPAOggVorbisDecoderSource.Create(TFileStream.Create(lfilename, fmOpenRead or fmShareDenyNone), TRUE);
  if not assigned(fdest) then
{$IFDEF UNIX}
    fdest := TPAPulseDestination.Create;
{$ELSE}
    fdest := TPAMMDestination.Create;
{$ENDIF}
...
procedure tmainfo.mainfo_onclose(const sender: TObject);
begin
  logln('[DEBUG] mainfo_onclose');
  fdest.Terminate;
  fdest.WaitFor;
  fdest.free;
  fsource.free;
...
procedure tmainfo.mainfo_onterminatequery(var terminate: Boolean);
begin
  logln('[DEBUG] mainfo_onterminatequery');
  fdest.Terminate;
  fdest.WaitFor;
  fdest.free;
  fsource.free;
